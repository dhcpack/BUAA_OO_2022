...
public class Output {
    ...
    public static synchronized void arrive(int floor, int elevatorId) {
        TimableOutput.println(
                String.format("ARRIVE-%c-%d-%d", (char) ('A' + elevatorId), floor, elevatorId + 1));
    }

    public static synchronized void open(int floor, int elevatorId) {
        TimableOutput.println(
                String.format("OPEN-%c-%d-%d", (char) ('A' + elevatorId), floor, elevatorId + 1));
    }

    public static synchronized void close(int floor, int elevatorId) {
        TimableOutput.println(
                String.format("CLOSE-%c-%d-%d", (char) ('A' + elevatorId), floor, elevatorId + 1));
    }

    public static synchronized void in(int personId, int floor, int elevatorId) {
        TimableOutput.println(
                String.format("IN-%d-%c-%d-%d", personId, (char) ('A' + elevatorId), floor,
                        elevatorId + 1));
    }

    public static synchronized void out(int personId, int floor, int elevatorId) {
        TimableOutput.println(
                String.format("OUT-%d-%c-%d-%d", personId, (char) ('A' + elevatorId), floor,
                        elevatorId + 1));
    }
}