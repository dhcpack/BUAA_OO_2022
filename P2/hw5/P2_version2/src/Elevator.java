...
public class Elevator implements Runnable {
    ...
    private void strategyLook() {
        while (true) {
            ...
            if ((peopleWaitElevator.size() + peopleOnElevator.size()) == 0 &&
                    requestTransfer.isEnd()) {
                // 可能此时已经end了
                ...
                if (requestTransfer.isEnd() && peopleWaitElevator.get(0) == null) {  // 可能等一会等到了end
                    // System.out.println(Thread.currentThread().getName() + " close");
                    return;
                }
            }
    ...
    private boolean changeDirection() {
        for (PersonRequest personRequest : peopleWaitElevator) {
            if ((personRequest.getFromFloor() > currentFloor && direction == UP) ||
    ...
    private boolean hasIn(boolean hasOut) {
        ...
        for (PersonRequest personRequest : peopleWaitElevator) {
            if (personRequest.getFromFloor() == currentFloor &&
                    ((personRequest.getToFloor() - currentFloor > 0 && direction == UP) ||
                            (personRequest.getToFloor() - currentFloor < 0 && direction == DOWN))) {
                return true;
            }
        }
        return false;
