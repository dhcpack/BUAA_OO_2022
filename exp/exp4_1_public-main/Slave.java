...
public class Slave implements Runnable {
    ...
    private void settleTask(TaskRequest taskRequest) {
        taskRequest.setDone(true); // TODO(1) ok
    }
    ...
    public void run() {
        while (true) {
            ...
            if (hasWork()) {  // TODO(2) ok
                TaskRequest taskRequest = publicList.fetchTask(id);
                Printer.println(
                        String.format("Slave %d fetches task %d", this.id, taskRequest.getId()));
                try {
                    Thread.sleep((long) (taskRequest.getTime() * 100));  // TODO(3) ok
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
                settleTask(taskRequest);  // TODO(4) ok
                Printer.println(
                        String.format("Slave %d settles task %d", this.id, taskRequest.getId()));
            } else {
                try {
                    Thread.sleep(1000L);
                } catch (InterruptedException e) {
