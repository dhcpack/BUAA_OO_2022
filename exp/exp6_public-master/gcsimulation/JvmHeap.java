...
public class JvmHeap extends MyHeap<MyObject> {
    ...
    /*@ public normal_behavior
      ...
      @ ensures (\forall int i; 1 <= i && i <= size;
                ...
                elements[i].getId() != ObjectId.get(j)) ==> (elements[i] == \old(elements[i])));
      @ [1]; 要求：定义后置条件，表示调用此方法前后，elements数组中下标大于等于1且小于等于size，  // finish 1
    ...
    public void removeUnreferenced() {
        ArrayList<MyObject> remain = new ArrayList<>();
    ...
    public /*@ pure @*/ MyObject getYoungestOne() {
        ...
        MyObject result = (MyObject) getElementData()[1];
        for (int i = 1; i <= getSize(); i++) {
            MyObject mo = (MyObject) getElementData()[i];
            if (result.compareTo(mo) > 0) {
                result = mo;
            }
        }
        return result;
