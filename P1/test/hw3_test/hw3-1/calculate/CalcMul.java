...
public class CalcMul implements Calculate {
    ...
    public static final CalcMul ONE = CalcMul.newDec(BigInteger.ONE);
    ...
    public String toString() {
        ...
        this.sins.forEach(new BiConsumer<Calculate, Integer>() {
            public void accept(Calculate t, Integer u) {
                String inner = CalcMul.intoFactorString(t);
                inner = "sin(" + inner + ")";
                if (u == 1) {
                    segments.add(inner);
                } else {
        ...
        if (segments.size() < 1) {
            ...
            result = sign + String.join("*", segments);
        }
        return result;
    }

    public static String intoFactorString(Calculate t) {
        String inner = t.toString().replaceFirst("^\\+*", "");
        if (!isAFactor(inner)) {
            inner = "(" + inner + ")";
        }
        return inner;
    ...
    public static boolean isAFactor(String s) {
        Parser p = new Parser(s);
        Tokenize.parseFactor(p);
        return p.ended();
    }
