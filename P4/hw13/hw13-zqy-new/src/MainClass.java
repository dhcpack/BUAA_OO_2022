...
public class MainClass implements UserApi {
    public static void main(String[] args) throws Exception {
        AppRunner.newInstance(MainClass.class).run(args);
    }
    ...
    public MainClass(UmlElement... elements) {
        // sort by dependency level
        Arrays.sort(elements, (e1, e2) -> Integer.compare(//
                Utility.elementClassLevel(e1), Utility.elementClassLevel(e2)));
        // read all elements
    ...
    private MyClass findClassByName(String className)
        ...
        for (MyClass cls : classes.values()) {
            if (cls.getUmlClass().getName().equals(className)) {
                if (result != null) {
                    // already found
    ...
    public Map<Visibility, Integer> getClassOperationVisibility(String className, String methodName)
        ...
        findClassByName(className).getOps().stream().map(id -> ops.get(id).getUmlOp())
                .forEach(uop -> {
                    if (uop.getName().equals(methodName)) {
    ...
    public List<Integer> getClassOperationCouplingDegree(String className, String methodName)
        ...
        List<MyOp> opList = queryCls.getOps().stream() //
                ...
                .filter(op -> op.getUmlOp().getName().equals(methodName)) // with name
